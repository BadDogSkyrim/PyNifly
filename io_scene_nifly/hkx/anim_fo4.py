...
def _build_anim_data_section(anim: AnimationData, name_offs: Dict[str, int]) -> Tuple[bytes, '_FixupBuilder']:
    ...
    # All events go on track 0, which is what the game ships: of 311
    # annotated vanilla FO4 animations, every one puts its events there and
    # leaves every annotation track name empty.
    annotations = list(anim.annotations or [])
    if annotations and num_tracks == 0:
        raise ValueError("Cannot export annotations: animation has no tracks.")
    ...
    for i in range(num_tracks):
        ...
        count = len(annotations) if i == 0 else 0
        annot_array_offsets.append(write(_hkarray(count)))

    # hkaAnnotation payload for track 0. Each event is 0x10 bytes: float
    # time, 4 bytes of padding, then an hkStringPtr to the text.
    if annotations:
        align16()
        annot_events_rel = rel()
        fx.add_local(annot_array_offsets[0], annot_events_rel)
        annot_event_offsets = []
